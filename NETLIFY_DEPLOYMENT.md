...
### 2. Aligo SMS API 계정 설정

1. **Aligo 회원가입**: https://smartsms.aligo.in/
2. **API 키 발급받기**:
   - 로그인 → 우측 상단 "내정보" → "API KEY" 확인
   - USER ID 확인
3. **발신번호 등록**:
   - "환경설정" → "발신번호 관리" → 070-7106-1658 등록 및 인증
...
| `ALIGO_SENDER` | `07071061658` | 발신번호 (하이픈 제거) |
| `OWNER_PHONE` | `070-7106-1658` | 사장님 연락처 (SMS 수신번호) |
...
1. 웹사이트에서 "상담문의" 버튼 클릭
2. 상담 신청 폼 작성 및 제출
3. **사장님 번호(070-7106-1658)로 SMS 수신 확인** ← 가장 중요!
...
### SMS가 발송되지 않는 경우
1. **환경 변수 확인**: Netlify 환경 변수가 올바르게 설정되었는지 확인
2. **Aligo 잔액 확인**: Aligo 계정 잔액이 충분한지 확인
3. **발신번호 확인**: 070-7106-1658이 Aligo에 등록 및 인증되었는지 확인
